...
public class VisitByViews {
    ...
    private static class reducer extends Reducer<Text, IntWritable, Text, IntWritable> {
        ...
        protected void reduce(Text key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            ...
            Text web = new Text(key.toString().split("\\s+")[1]);
            if (visits.containsKey(web)) {
                visits.put(web, count+(int)visits.get(web));
            }else {
                visits.put(web, count);
            }
        }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        job.setMapperClass(views.VisitByViews.mapper.class);
        job.setReducerClass(views.VisitByViews.reducer.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(IntWritable.class);
        job.setPartitionerClass(views.VisitByViews.myPartitioner.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(IntWritable.class);
        job.setNumReduceTasks(1);
        Path input = new Path("./src/views/input");
        FileInputFormat.setInputPaths(job, input);
        Path output = new Path("./src/views/output");
